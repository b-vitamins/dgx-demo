...
So:
- Debug on 1 GPU with short `--time`, small `--max_steps`, and small `--dataset_size`.
...
- straightforward accounting and isolation

### B) Pack trials into a multi-GPU allocation
- request 4 GPUs
- run 4 independent 1-GPU trials in parallel inside one job
- do not oversubscribe CPU/RAM
- pin each trial to its own GPU (this repo does this with `CUDA_VISIBLE_DEVICES`)
