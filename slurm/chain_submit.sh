...
for i in $(seq 1 "${N}"); do
  if [[ -z "${jid_prev}" ]]; then
    jid_prev=$(sbatch --export=ALL,RUN_ID="${RUN_ID}" slurm/02_train_1gpu_12h_chain.sbatch | awk '{print $4}')
    echo "[chain] segment ${i} -> job ${jid_prev}"
  else
    jid=$(sbatch --dependency=afterany:"${jid_prev}" --export=ALL,RUN_ID="${RUN_ID}" slurm/02_train_1gpu_12h_chain.sbatch | awk '{print $4}')
